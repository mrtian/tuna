...
class CameraPickerState extends State<CameraPicker>
  ...
  // 区分前后的相机实例
  Map<String,List<CameraDescription>> camerasMap = {};
  ...
  void initCameras([CameraDescription? cameraDescription]) {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      cameras.forEach((CameraDescription ca) {
        if(ca.lensDirection == CameraLensDirection.back){
          if(!camerasMap.containsKey('back')){
            camerasMap['back'] = [ca];
          }else{
            camerasMap['back']!.add(ca);
          }
        }else if(ca.lensDirection==CameraLensDirection.front){
          if(!camerasMap.containsKey('front')){
            camerasMap['front'] = [ca];
          }else{
            camerasMap['front']!.add(ca);
          }
        }
      });
      }
  ...
  void switchCameras() {
    ...
    // }
    // initCameras(currentCamera);
    if(widget.cameraDirection=='front'){
      if(currentCameraIndex==0){
        currentCameraIndex = 1;
        initCameras(camerasMap['back']![0]);
      }else{
        currentCameraIndex = 0;
        initCameras(camerasMap['front']![0]);
      }
    }else{
      if(currentCameraIndex==0){
        currentCameraIndex = 1;
